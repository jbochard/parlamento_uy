...
class SenadoresPedidosInformeWorker(WorkerScrap):
    ...
    def execute(self):
        print('\tImportando pedidos de informe de %s' % self.id_senador)

        file = get_html('https://parlamento.gub.uy/camarasycomisiones/legisladores/%s/pedidosInf-legislador?Fecha[min][date]=%s&Fecha[max][date]=%s' % (self.id_senador, self.date_from, self.date_to))
        h = BeautifulSoup(file, 'lxml')
        for html_row in find_all(find(h, 'tbody'), 'tr'):
            id_senador = self.id_senador
            ...
            for organismo_html in organismos_html:
                ...
                DBScraping().insert('pedidos_informe', id_proyecto, {'id_proyecto': id_proyecto, 'id_senador': id_senador, 'fecha': fecha, 'organismo': organismo})
